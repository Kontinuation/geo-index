...
/// Test with larger dataset and more neighbors
#[test]
fn test_neighbors_geometry_larger_dataset() {
    let options = RandomGeometryOptions {
        ...
        size_range: (5.0, 50.0),
        vertices_per_polygon_range: (4, 12),
    };

    for seed in 0..5 {
        // Index polygons, query with points
        let indexed_geometries = generate_random_polygons(seed, 200, &options);
        let query_geometries = generate_random_points(seed + 1000, 20, &options);
        ...
        for (query_idx, query_geom) in query_geometries.iter().enumerate() {
            verify_neighbors_geometry(
                ...
                10,
                &format!(
                    "larger_dataset_polygon_index_point_query seed={} query={}",
                    seed, query_idx
                ),
            );
        }

        // Index polygons, query with polygons
        let query_geometries = generate_random_polygons(seed + 2000, 20, &options);
        for (query_idx, query_geom) in query_geometries.iter().enumerate() {
            verify_neighbors_geometry(
                query_geom,
                &indexed_geometries,
                10,
                &format!(
                    "larger_dataset_polygon_index_polygon_query seed={} query={}",
                    seed, query_idx
                ),
            );
        }
...
/// Test edge case: query geometry that overlaps indexed geometries
#[test]
fn test_neighbors_geometry_overlapping() {
    let options = RandomGeometryOptions {
        bounds: Rect::new(Coord { x: 0.0, y: 0.0 }, Coord { x: 50.0, y: 50.0 }),
        size_range: (5.0, 20.0), // Larger polygons to ensure overlaps
        vertices_per_polygon_range: (4, 8),
    };

    for seed in 0..10 {
        let indexed_geometries = generate_random_polygons(seed, 30, &options);
        let query_geometries = generate_random_polygons(seed + 1000, 10, &options);
        ...
        for (query_idx, query_geom) in query_geometries.iter().enumerate() {
            verify_neighbors_geometry(
                ...
                &indexed_geometries,
                5,
                &format!("overlapping seed={} query={}", seed, query_idx),
            );
        }
    }
}
...
fn test_minimal_polygon_ordering_bug() {
    ...
    let rtree_results = tree.neighbors_geometry(&query_geom, Some(3), None, &metric, &accessor);
    ...
    let actual_indices: Vec<usize> = rtree_results.iter().map(|&idx| idx as usize).collect();
...
fn test_elongated_query_polygon_bug() {
    ...
    let rtree_with_distances: Vec<(usize, f64)> = rtree_results
        ...
        .map(|&idx| {
            (
                idx as usize,
                Euclidean.distance(&query_geom, &indexed_geometries[idx as usize]),
            )
        })
        .collect();
